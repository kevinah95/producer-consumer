...
int main(int argc, char *argv[]) {
  ...
  /* make * shelf shared between processes*/
    ...
    *SUSPEND = 1;
     while(*SUSPEND > 0){
       sleep(ran_expo(MEDIUM_SECONDS));
       char message[200];
       // print local time
      if (hours < 12)	// before midday
        sprintf(message, "Message from producer - PID: %i - Date and time: %02d/%02d/%d %02d:%02d:%02d am - Random number: %i\n", getpid(), day, month, year, hours, minutes, seconds, getRandomNumber());
      else	// after midday
        sprintf(message, "Message from producer - PID: %i - Date and time: %02d/%02d/%d %02d:%02d:%02d pm - Random number: %i\n", getpid(), day, month, year, hours, minutes, seconds, getRandomNumber());
      MESSAGE_COUNTER++;
       printf(message);
        /*sem_wait(avail);
        sleep(rand()%2+1);
